    ...
    <AppShell>
      ...
      <Tabs
        ...
        tabs={[
          ...
          { key: 'organizations', label: 'Organization settings' },
        ]}
      />
...
function OrganizationSettings() {
  ...
  const toast = useToast();
  const { memberships, currentOrganizationId, setCurrentOrganization } = useWorkspace();
  const { data: organizations, isLoading, isError, error, refetch } = useOrganizations();
  const leaveMut = useLeaveOrganization();
  const [createOpen, setCreateOpen] = useState(false);
  const [inviteOrganization, setInviteOrganization] = useState<Organization | null>(null);
  const [leaveOrganization, setLeaveOrganization] = useState<Organization | null>(null);

  const membershipById = useMemo(
    () => new Map(memberships.organizations.map((membership) => [membership.id, membership])),
    ...
    <>
                ...
                >
                  ...
                  <div className="organization-row__body">
                    <div className="organization-row__title">
                      ...
                      {isCurrent && <Badge variant="teal">Current workspace</Badge>}
                      {membership && (
                        <Badge variant={membership.role === 'CORE_BOARD' ? 'amber' : 'neutral'}>
                          {membership.role === 'CORE_BOARD' ? 'Core Board' : 'Member'}
                        </Badge>
                      )}
                    </div>
                    <p>{organization.description || 'No organization description.'}</p>
                  </div>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="danger" loading={leaveMut.isPending} onClick={confirmLeave}>
              Leave organization
            </Button>
          </>
        }
      >
        <div className="settings-leave-warning">
          ...
          <p>
            Core Board members cannot leave when they are the organization&apos;s last
            active Core Board member.
          </p>
        </div>
      </Modal>
    </>
